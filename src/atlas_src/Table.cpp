...
unsigned int Table::EncodeStream(string& scriptbuf, unsigned int& BadCharOffset)
{
		...
		while(i)
		{
			...
			for(unsigned int j = 0; j < EndTokens.size(); j++)
				if(EndTokens[j] == subtextstr)
				{
					...
					TblString.Text.erase( TblString.Text.begin(), TblString.Text.end() );
					break; // Only once
				}
	...
	//scriptbuf.clear();
	scriptbuf.erase( scriptbuf.begin(), scriptbuf.end() );
...
int Table::OpenTable(const char* TableFilename)
{
	...
	while(!TblFile.eof())
	{
		//HexVal.clear();
		//TextString.clear();
		HexVal.erase( HexVal.begin(), HexVal.end() );
...
inline bool Table::parseendstring(ifstream& file)
{
	...
	if(testch == '\n') // Must be a blank value string (/<end>)
	{
		...
		textstr = hexstr;
		//hexstr.clear();
		hexstr.erase( hexstr.begin(), hexstr.end() );
	}
